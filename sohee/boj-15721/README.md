...
###아이디어
  <p>1.인원수와 T,0 or 1(번 or 데기)를 입력 받는다.</p>
  <p>2.처음 2번은 무조건 뻔-데기로 반복하니깐(뻔-데기-뻔-데기) 2번 반복하는 반복문을 사용하여 리스트에 추가해준다.</p>
  <p>3.그 이후로 부터는 인원수에 따라 뻔이 N+1개만큼 반복하게 된다.따라서 그 만큼 반복하는 반복문을 써서 뻔을 의미하는 0을 리스트에 추가한다.</p>
  <p>4.데기도 뻔과 같은 방법으로 해준다.</p>
  <p>5.뻔의 개수가 T번째의 수보다 커지거나 같아지게 된다면  그 요소에 해당되는 인덱스를 인원수만큼 모듈러를 사용하여 출력한다.</p>
